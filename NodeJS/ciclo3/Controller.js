...
app.post('/itempedidos', async(req,res)=>{
    ...
    await itempedido.create(item).then(function(){
        return res.json({
            error: false,
...
app.get('/consultarServicoItem/:id', async(req,res)=>{
    await servico.findAll({
...
app.get('/produto/:id/comprados', async(req,res)=>{
...
app.get('/cliente/:id/comprou', async(req,res)=>{
    await cliente.findAll({
        where: {id: req.params.id}
    })
    .then(clientes =>{
...
app.get('/listarProdutos/:id', async(req,res)=>{
    await produto.findAll({
        where: {id:req.params.id}
    }).then(function(produtos){
        res.json({produtos})
...
app.put('/atualizarItemCompra/:id/editarItem/:idd', async(req,res)=>{
    const itemCompra = {
        quantidade: req.body.quantidade,
        valor: req.body.valor
    };
    ...
    if(!await produto.findByPk(req.params.idd)){
        return res.status(400).json({
            error: true,
...
app.get('/excluirItemCompra/:id/excluir/:idd', async(req,res)=>{
    if(!await compra.findByPk(req.params.id)){
        return res.status(400).json({
            error: true,
            message: 'A compra não existe!'
